...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT p.post_id, p.content, p.image_path, p.created_at, p.user_email, p.likes, 
        ...
        LEFT JOIN follows f ON u.email = f.followed_email AND f.follower_email = ?
        LEFT JOIN post_likes pl ON pl.post_id = p.post_id AND pl.user_email = ?
        WHERE f.follower_email IS NOT NULL OR p.user_email = ?
    ...
    $followCheckStmt = $pdo->prepare("
        SELECT COUNT(*) FROM follows WHERE follower_email = ?
    ");
    $followCheckStmt->execute([$loggedInUserId]);
    $followingCount = $followCheckStmt->fetchColumn();
...
<div class="sliding-menu" id="slidingMenu">
  <div class="menu-items">
    <a href="index.php" class="menu-item current"><i class="fas fa-home"></i> Home</a>
      ...
      <?php foreach ($posts as $post): ?>
        ...
        <div class="post-item">
          <div class="post-header">
            <a href="account.php?user=<?php echo urlencode($post['user_email']); ?>"></a>
...
<script>
  // Update the JavaScript section to correctly handle the like functionality:

  document.addEventListener('DOMContentLoaded', function() {
  // Menu Toggle Functionality (keep as is)
